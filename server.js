...
function handleJoin(ws, userId, message) {
    ...
    if (!rooms.has(roomId)) {
        rooms.set(roomId, { users: new Map(), presenter: null });
    ...
    if (room.presenter) {
        ...
        if (presenter) {
            ...
            // Tell the presenter to send a new offer to this specific viewer
            sendToUser(room.presenter, {
                type: 'new-viewer',
                viewerId: userId,
                viewerName: username
            });
            console.log(`📢 Notified presenter about new viewer: ${username}`);
        }
    }
}
